...
- First rebase to the unsigned image, to get the proper signing keys and policies installed:
  ```
  rpm-ostree rebase ostree-unverified-registry:ghcr.io/mondrethos/monolith-gnome:latest
  ```
...
The `latest` tag will automatically point to the latest build. That build will still always use the Fedora version specified in `recipe-gnome.yml`, so you won't get accidentally updated to the next major version.
...
```bash
cosign verify --key cosign.pub ghcr.io/mondrethos/monolith-gnome
```
